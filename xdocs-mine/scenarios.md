...
1. GitHub URLS: if it's a file, direct fetch it (any extension)

2. If URL matches a prefix in `direct-fetch-rules.toml`, direct fetch it's twin. `<source_url>` must be the non-twin.
    ```markdown
...
3. If URL ends in [md|mdx|qmd|txt], direct fetch
...
4. Fallback to firecrawl scrape
    - https://lefthook.dev/configuration/colors/
    - https://wisprflow.ai/ (what happens?)
    - https://does-not-exist.com/hello (what happens?)

5. Edge case: GitHub should direct fetch any file, but should I go to the trouble of direct fetching non-github urls that are too just files like .jpg, .svg, .pdf. Is this over engineering. What happens now if I do it.
...
6. Edge case: firecrawl content not the same as direct fetch. So what happens in the scenario when you put a "non-md" looking URL in, and then later you put the ".md" at the end URL. two source entries right? this seems okay to me. not sure.

7. Edge case: `md-append` does not catch ULRs that end in .pdf, jpg etc. IT falls through to firecrawl. But these files should be directly fetch. but then how would you title them anyhow. this is such a remote use case. but how should I handle.

8. Handle when I get 404s (even though not all sites give them). This came up when I was testing URLs with different casing.

9. Don't allow curating the root .toml prefixes
